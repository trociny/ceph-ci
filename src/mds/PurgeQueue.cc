...
#if 0
  // We don't know how many ops the next item will require until popping
  // it off the Journaler, so make a reasonable guess based on a large
  // file plus its backtrace plus one old pool.  This will lead us
  // to almost always stay within our ops limit.  We will never exceed
  // our ops limit by more than the largest size of old_pools minus one.
  const uint32_t typical_ops_per_item = cct->_conf->filer_max_purge_ops + 1 + 1;

  if (ops_in_flight + typical_ops_per_item >= max_purge_ops) {
